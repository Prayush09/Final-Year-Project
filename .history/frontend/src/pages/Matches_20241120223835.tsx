...
export default function Matches() {
  ...
  const fetchMatches = async () => {
    try {
      const response = await axios.get(`http://localhost:3000/api/match/`, { withCredentials: true });
  
      // Now that the response is an array of matches, directly set them
      const fetchedMatches = response.data || []; // Default to empty array if undefined
  ...
  const handleAccept = async (matchedUserId: string) => {
    try {
      await axios.put(
        'http://localhost:3000/api/matches/acceptMatch',
  ...
  const handleReject = async (matchedUserId: string) => {
    try {
      await axios.put(
        'http://localhost:3000/api/matches/rejectMatch',
        { matched_user_id: matchedUserId },
        { withCredentials: true }
      );
      setMatches((prev) =>
  ...
  const handleDelete = async (matchedUserId: string) => {
    try {
      await axios.delete('http://localhost:3000/api/matches/deleteMatch', {
