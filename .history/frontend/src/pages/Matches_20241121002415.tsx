...
export default function Matches() {
  ...
  const fetchMatches = async () => {
    try {
      ...
      // Set the fetched matches directly (no need to filter by score)
  ...
  .filter((match) => {
    ...
    return (
      ...
      profile?.location_preferences.toLowerCase().includes(searchQuery.toLowerCase())
    );
  })
  .sort((a, b) => {
    const profileA = profiles.get(a.matched_user_id);
    const profileB = profiles.get(b.matched_user_id);

    switch (filter) {
      case 'age':
        return (profileA?.age ?? 0) - (profileB?.age ?? 0);
      default:
        return 0;
    }
  });
