...
const { user } = useAuth(); // Access Zustand store
...
export default function ProfileForm() {
  ...
  sync function onSubmit(data: ProfileFormValues) {
    const { user } = useAuth(); // Get the user from Zustand store
    const user_id = user ? user.id : null; // Extract user_id from the user object
    ...
    const dataWithUserId = { ...data, user_id }; // Merge data with user_id
  
    setLoading(true); // Set loading state to true before making the request
    try {
      // Send the data with user_id to the backend
      await axios.post('http://localhost:3000/api/profiles/create', dataWithUserId, {
