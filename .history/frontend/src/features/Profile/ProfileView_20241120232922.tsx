...
export default function ViewProfile() {
  ...
  axios.get(`http://localhost:3000/api/profiles/userId?email=${email}`)
    .then((response) => {
      const data = response.data;
      isProfileComplete = data.profile_completed;
    });

  useEffect(() => {
    if (!user_id) {
      console.error("Please log in again!, LOCALSTORAGE_USERID_NOTFOUND");
      return;
    }
      ...
      <div className="relative w-full"> 
        ...
        <Card className="relative bg-white dark:bg-black shadow-xl m-[2px] backdrop-blur-sm">
          <CardHeader>
            ...
            {/* Added name here */}
            <div className="text-lg text-violet-600 dark:text-violet-300 font-semibold">
              {user.name}
            </div>
            <CardDescription>
