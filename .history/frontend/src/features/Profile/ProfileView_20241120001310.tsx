  ...
  return (
    <div className="max-w-3xl mx-auto p-6">
      <Card className="profile-container bg-white dark:bg-transparent shadow-xl">
        <CardHeader>
          <CardTitle className="text-2xl font-bold bg-gradient-to-r from-indigo-600 to-[#8D4DE8]-600 bg-clip-text text-transparent">
            Your Profile
          </CardTitle>
          <CardDescription>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <p className="text-violet-400">{user.age}</p>
              </div>

              <div>
                <h3 className="font-semibold">Gender:</h3>
                <p className="text-violet-400">{user.gender}</p>
              </div>
            </div>

            <div>
              <h3 className="font-semibold">About Me:</h3>
              <p className="text-[#8D4DE8]">{user.bio}</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <h3 className="font-semibold">Cleanliness Level:</h3>
                <p className="text-violet-400">{user.cleanliness}</p>
              </div>

              <div>
                <h3 className="font-semibold">Noise Tolerance:</h3>
                <p className="text-violet-400">{user.noise_tolerance}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <p className="text-violet-400">{user.sleep_schedule}</p>
              </div>

              <div>
                <h3 className="font-semibold">Monthly Budget (₹):</h3>
                <p className="text-violet-400">{user.budget}</p>
              </div>
            </div>

            <div>
              <h3 className="font-semibold">Preferred City:</h3>
              <p className="text-violet-400">{user.location_preferences}</p>
            </div>
          </div>
        </CardContent>
      </Card>
