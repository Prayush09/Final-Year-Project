...
export default function Dashboard() {
  ...
  const fetchMatches = async () => {
    try {
      ...
      const matchedUserIds = new Map(
        userMatches.data.map((match: Match) => [match.matched_user_id, match])
      );

      // Fetch matches for each matched user
      const matchPromises = Array.from(matchedUserIds.keys()).map(async (matched_user_id) => {
        ...
        return {
          ...
          mutualMatch: matchResponse.data.some(
            (m: Match) => m.matched_user_id === user_id && m.status === 'accepted'
          )
        };
      });
      ...
      // Separate into incoming requests and current matches
  ...
  const handleAccept = async (matchedUserId: string) => {
    try {
      ...
      // Move match from incoming to current
      const acceptedMatch = incomingRequests.find(m => m.matched_user_id === matchedUserId);
      if (acceptedMatch) {
