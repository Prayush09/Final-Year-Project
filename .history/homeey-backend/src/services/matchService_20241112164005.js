...
const { acceptMatch } = require('../controllers/matchController');
...
const matchService = {
    ...
    async calculateMatchScore(userProfile, otherProfile){
        try{
            ...
            const response = await fetch("https://ec77-35-194-249-201.ngrok-free.app/predict", {
                method:"POST",
                headers:{
    ...
    async acceptMatch(user_id, matched_user_id){
        try{
            const updatedMatch = await Match.acceptMatchMatch(user_id, matched_user_id);
