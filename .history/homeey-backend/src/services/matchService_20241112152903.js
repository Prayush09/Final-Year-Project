...
const matchService = {
    ...
    async updateMatch(user_id, matched_user_id){
        const match = await Match.getMatch(user_id, matched_user_id);

        try{
            if(match[0].status === 'pending'){
                match[0].status = 'accepted';//make the match accepted
            }
            await Match.updateMatch(match);
        }catch(error){
            console.error("Error occurred while updating the match: ", error);
        }
    },
