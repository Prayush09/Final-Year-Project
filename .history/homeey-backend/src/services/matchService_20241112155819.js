...
const matchService = {
    ...
    // delete all matches for user -> then create new matches for user... 
    async updateMatches(user_id){
        try{
            const updateMatches = await Match.updateMatches(user_id);

            if(updatedMatch){
                await this.findMatchForUser(user_id);//create new matches...
            }else{
                console.log("All matches are already upto date...");
            }
        }catch(error){
            console.error(`Some error occurred during match updates for ${user_id} `, error);
        }
    },
