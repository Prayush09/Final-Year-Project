...
const matchService = {
    ...
    async calculateMatchScore(userProfile, otherProfile){
        try{
            ...
            const response = await fetch("https://b4ad-35-229-204-215.ngrok-free.app/predict", {
                method:"POST",
                headers:{
            ...
            console.log("If the match is 0, then the location is different else the number will be present: ", data);
            ...
            return Math.floor(Math.random() * 100 < 100 ? Math.random()*100 + 1 : Math.random()*100 + 0); //fall back on dummy score if error occurs 
        }
    },
